...
#########################################
table(treated_CAF.obj$orig.ident, treated_CAF.obj$ENG_status)


## Build donor x ENG_status pseudobulk counts ----------------------------------
treated_CAF.obj <- JoinLayers(treated_CAF.obj, assay = "RNA")
counts <- GetAssayData(treated_CAF.obj, assay = "RNA", layer = "counts")

meta   <- treated_CAF.obj@meta.data
...
pdf('results/Treated_ENG_donor_pseudobulk_HallMark_GSEAbar.pdf',
    width = 6, height = 8)
NEG_plot_pseudo
dev.off()

pdf('results/Treated_ENG_donor_pseudobulk_enrichmentPlot.pdf',
    width = 4, height = 2.5)
enrich_plots_pseudo
dev.off()
...
write.csv(
  ...
  file = "results/Treated_ENG_donor_pseudobulk_GSEA.csv",
